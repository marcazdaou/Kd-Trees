...
public class KdTreePointST<Value> implements PointST<Value> {
    private Node root;  // roots
    private int n;  // n

    // Constructs an empty symbol table.
    public KdTreePointST() {
        n = 0;  // instance variable
        root = null;
    }
    ...
    public Iterable<Point2D> points() {
        LinkedQueue<Node> q1 = new LinkedQueue<>(); // store Node
        LinkedQueue<Point2D> q2 = new LinkedQueue<>(); // store Point
        q1.enqueue(root);
        while (!q1.isEmpty()) {
            Node x = q1.dequeue();
            q2.enqueue(x.p);
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        range(root, rect, q);  // range(root, rect, q)
        return q;
    }
    ...
    public Point2D nearest(Point2D p) {
        ...
        if (root != null) {
            ...
            return nearest(root, p, nearest, lr);
        } else {
    ...
    public Iterable<Point2D> nearest(Point2D p, int k) {
        MaxPQ<Point2D> pq = new MaxPQ<>(k, p.distanceToOrder());
    ...
    private Node put(Node x, Point2D p, Value value, RectHV rect, boolean lr) {
        ...
        if (x.p.equals(p)) {
            ...
            RectHV myrectangle = lr ? new RectHV(rect.xMin(), rect.yMin(), x.p.x(), rect.yMax())
                    : new RectHV(rect.xMin(), rect.yMin(), rect.xMax(), x.p.y());
            x.lb = put(x.lb, p, value, myrectangle, !lr);
        } else {
            RectHV myrectangle = lr ? new RectHV(x.p.x(), rect.yMin(), rect.xMax(), rect.yMax())
                    : new RectHV(rect.xMin(), x.p.y(), rect.xMax(), rect.yMax());
            x.rt = put(x.rt, p, value, myrectangle, !lr);
        }
        return x;
    }
    ...
    private Value get(Node x, Point2D p, boolean lr) {
        ...
        if (x.p.equals(p)) {  // equals
            return x.value;
        }
        if ((lr && p.x() < x.p.x()) || (!lr && p.y() < x.p.y())) {
            return get(x.lb, p, !lr);
        } else {
            return get(x.rt, p, !lr);  // recursive
        }
    }
    ...
    private Point2D nearest(Node x, Point2D p, Point2D nearest, boolean lr) {
        ...
        Point2D best = nearest;

        // Update nearest if x is closer to p than nearest
        if (x.p.distanceSquaredTo(p) < best.distanceSquaredTo(p)) {
            best = x.p;
        }

        // Recursively search the half of the tree that contains p
        if (lr && p.x() < x.p.x() || !lr && p.y() < x.p.y()) {
            best = nearest(x.lb, p, best, !lr);  // best
            best = nearest(x.rt, p, best, !lr);
        } else {
            best = nearest(x.rt, p, best, !lr);
            best = nearest(x.lb, p, best, !lr);
        }
    ...
    private void nearest(Node x, Point2D p, int k, MaxPQ<Point2D> pq, boolean lr) {
        if (x == null || pq.size() >= k
                && x.rect.distanceSquaredTo(p) >= pq.max().distanceSquaredTo(p)) {
            return;
        }
